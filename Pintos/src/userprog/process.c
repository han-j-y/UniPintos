...
unsigned* my_hash_func (const struct hash_elem *elem, void *aux)
{
	struct supp_page* spage = hash_entry (elem, struct supp_page, elem);
	return hash_bytes (&spage->user_addr, sizeof (&spage->user_addr));
}

bool* my_hash_less (const struct hash_elem *a,
					const struct hash_elem *b, void *aux)
{
	struct supp_page* p1 = hash_entry (a, struct supp_page, elem);
	struct supp_page* p2 = hash_entry (b, struct supp_page, elem);

	return (p1->user_addr < p2->user_addr);
}
...
start_process (void *file_name_)
{
  ...
  hash_init (&thread_current()->supp_table, my_hash_func, my_hash_less, NULL);
  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      /* Using supp table */
      struct supp_page* spage = create_supp_page (file, ofs, page_read_bytes, page_zero_bytes, writable, upage);

	  if (!add_supp_page (&thread_current()->supp_table, spage))
		  return false;
	  /* Advance. */
