...
struct mapping* addr_to_map (void *addr)
{
	...
	while (hash_next (&i))
	{
		struct mapping *m = hash_entry (hash_cur (&i), struct mapping, elem);
		if (m->addr <= pg_round_down (addr) && pg_round_down (addr) < m->addr + (m->num_pages * PGSIZE))
			return m;
	}
	return NULL;
}
